...
var rainfall = (function () {
    ...
    function geoFindMe() {
      ...
      function success(position) {
        ...
        vectorSource.forEachFeature(function(feat){
            var d = getDistanceFromLatLonInKm(latitude,longitude,feat.getGeometry().A[1],feat.getGeometry().A[0]);
            arr.push({
                'dev_id': feat.get('device_id'),
                'distance': d
            });
        });
        ...
        arr.forEach(function(data) {
             global_arr.push(data['dev_id']);
             plotARGs(data['dev_id'])
        });
    ...
    function colorize() {
        ...
        if (global_arr.length > 0){
            global_arr.forEach(function(dev_id) {
                plotARGs(dev_id)
            });
        }else{
            arr_id.forEach(function(dev_id) {
                plotARGs(dev_id)
            });
        }
    ...
    function plotARGs(dev_id) {
        ...
        getData.done(function (data) {
            ...
            if (data_len > 0) {
                ...
                rain_avg_f = rain_avg.toFixed(2);
                if ((accum > 50) && (rain_avg_f > 50)) {
                    x.play();
                    var options =  {
                        content: "Please monitor the rainfall condition in "+loc ,
                        style: "snackbar",
                        timeout: 0,
                        htmlAllowed: true,
                        onClose: function(){}
                ...
                //console.log(loc+'- '+dev_id+': '+rain_avg_f)
            }
            vectorSource.forEachFeature(function(feat){
    ...
    var displayFeatureInfo = function (pixel) {
        ...
        if (feature) {
            if (map.getView().getZoom() < 11) {
                var locs = feature.get('rain_intensity') > 0 ? 'Daily Rainfall Occurrence(%): ' +feature.get('rain_occurrence')+'<br/><small> Click to view details in graph.</small>'  : feature.get('proper_name')+'<br/>Daily Rainfall Occurrence(%): ' +feature.get('rain_occurrence');
                info.tooltip('hide')
    ...
    function initMap() {
        ...
        map.on('singleclick', function (evt) {
            ...
            if (feature) {
                ...
                var mun = feature.get('City_Municipality');
